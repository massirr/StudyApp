# Spec: Code Snippet Questions

## Purpose

Add a harder tier of quiz questions that embed a code block (Python, SQL, or Databricks-specific syntax) in the question body, so learners practice reading and reasoning about code before the exam.
...
Each of the 5 DP-750 topics SHALL have at least 4 MCQ questions that include `codeSnippet`. Questions MUST be grounded in official Microsoft Azure Databricks documentation and include at least one `sourceUrl`.

#### Scenario: Questions load for each topic
- **WHEN** the code-snippet quiz for any of the 5 DP-750 topics loads
- **THEN** at least 4 questions with `codeSnippet` SHALL be present for that topic
