# Spec: Short-Text Questions

## Purpose

Define the `shortText` question type: a short typed answer the app grades itself against a set of accepted answers, for exercises where the learner must produce a form rather than recognise it. It sits between the MCQ types (auto-graded, but recognition only) and `freeText` (typed, but self-graded because open-ended answers have many correct phrasings).
