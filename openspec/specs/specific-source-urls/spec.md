# Spec: Specific Source URLs

## Purpose

Ensure all quiz questions in `questions.ts` reference specific, directly relevant Microsoft Learn documentation pages rather than generic landing pages. Paired with this, the `sources.ts` registry must contain a corresponding entry (with label and usage note) for every URL used in questions, so that source validation and label display work end-to-end.
